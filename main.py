...
def main(token_list):
    banner = f"""
    {putih}==============================={hijau} Winnode Project TAP=TAP ONCHAIN BOT {putih}===============================
    {putih}                                                                                                           
    ...
    if not os.path.exists("config.json"):
        print(f"{merah}'config.json' file is not found !")
        sys.exit()
    ...
    for token in token_list:
        ...
        if not token_str:
            print(f"{kuning}Invalid token found in 'token.json' file!")
            continue

        if Onchain(token_str).is_expired():
            print(f"{kuning}Token has expired, skipping account!")
            continue

        process = Process(target=claim_account, args=(token_str, cfg))
...
def claim_account(token, cfg):
    onchain = Onchain(token)
    onchain.get_me()
    print("~" * 50)
    while True:
        ...
        onchain.click(cfg)
        print("~" * 50)
        onchain.countdown(cfg.interval)


if __name__ == "__main__":
    try:
        token_list = json.loads(open("token.json").read())
        if not isinstance(token_list, list) or len(token_list) == 0:
            print(f"{kuning}Please provide valid token data in 'token.json' file!")
            sys.exit()
